...
def run(question_train,labels_train,question_test,labels_test,tag):
    log.write("Tag: "+tag+"\n")
...
#    LDA_classifier=LatentDirichletAllocation()
    ...
    predictors=[('knn',KNN_classifier),('dt',DT_classifier)]
...
if __name__=='__main__':
     ...
     log=open('classification.txt','w') # test file 
     acc ={}
     for line in fin: #for each tag 
        tag=line.lower().strip()
        print(tag)
        question_train,labels_train=loadData('train2.txt',tag)
        question_test,labels_test=loadData('test2.txt',tag)
        run(question_train,labels_train,question_test,labels_test,tag)
     print("--- %s seconds ---" % round(time.time() - start_time, 2))
     log.write("Execution Time:--- %s seconds ---" % str(round(time.time() - start_time, 2)))
     
     top = dict(sorted(acc.items(), key=operator.itemgetter(1), reverse=True)[:10])
     
     print(top)
     log.write("\n\nTop Accuracies: " + str(top))
